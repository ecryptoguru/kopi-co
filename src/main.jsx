...
function MenuCard({ item, onAdd }) {
  return <article className="menu-card">
    <div className={`menu-image image-${item.id}`} role="img" aria-label={`${item.name} photo`} />
...
function Auth({ onAuth }) {
  ...
  const submit = (e) => { e.preventDefault(); if (!email || password.length < 6 || (mode === 'signup' && !name)) return setError('Please complete the form. Passwords need 6+ characters.'); const user = { name: mode === 'signup' ? name : (email.split('@')[0] || 'Kopi friend'), email, role: email.toLowerCase() === 'staff@kopi.co' ? 'staff' : 'customer' }; write('kopi-user', user); onAuth(user); window.location.hash = 'home' }
  return <main className="narrow-page"><div className="auth-card"><p className="kicker">Welcome in</p><h1>{mode === 'signin' ? 'Good to see you.' : 'Join the table.'}</h1><p className="muted">{mode === 'signin' ? 'Sign in to order, book, and play Kopi Catch.' : 'Create a local demo account for the full Kopi & Co. experience.'}</p><form onSubmit={submit} noValidate>{mode === 'signup' && <label>Name<input value={name} onChange={(e) => setName(e.target.value)} /></label>}<label>Email<input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@example.com" /></label><label>Password<input type="password" value={password} onChange={(e) => setPassword(e.target.value)} /></label>{error && <p className="error">{error}</p>}<button className="button" type="submit">{mode === 'signin' ? 'Sign in' : 'Create account'} <span>→</span></button></form><button className="switch-button" onClick={() => { setMode(mode === 'signin' ? 'signup' : 'signin'); setError('') }}>{mode === 'signin' ? 'Need an account? Sign up' : 'Already have an account? Sign in'}</button></div></main>
}

function Order({ user, cart, setCart }) {
  ...
  const place = () => { if (!cart.length) return; const orders = read('kopi-orders', []); orders.unshift({ id: Date.now(), user: user.name, items: cart, total, status: 'Preparing', created: new Date().toISOString() }); write('kopi-orders', orders); setCart([]); window.location.hash = 'confirmation' }
  return <main className="narrow-page order-page"><SectionHeading kicker="Order online">Your table, wherever you are.</SectionHeading><div className="order-layout"><div className="order-menu">{menuItems.map((item) => <MenuCard key={item.id} item={item} onAdd={(next) => setCart([...cart.filter((r) => r.item.id !== next.id), { item: next, qty: (cart.find((r) => r.item.id === next.id)?.qty || 0) + 1 }])} />)}</div><aside className="cart-panel"><h2>Your cart</h2>{!cart.length ? <p className="muted">Your cart is ready for something good.</p> : <>{cart.map((row) => <div className="cart-row" key={row.item.id}><div><strong>{row.item.name}</strong><span>{money(row.item.price)} each</span></div><div className="qty"><button onClick={() => update(row.item.id, -1)} aria-label="Decrease quantity">−</button><span>{row.qty}</span><button onClick={() => update(row.item.id, 1)} aria-label="Increase quantity">+</button></div></div>)}<div className="cart-total"><span>Total</span><strong>{money(total)}</strong></div><button className="button full" onClick={place}>Place order <span>→</span></button></>}</aside></div></main>
}

function Bookings({ user }) {
  const [form, setForm] = useState({ name: user?.name || '', party: '2', date: '', time: '', phone: '' }); const [done, setDone] = useState(false)
  const submit = (e) => { e.preventDefault(); if (Object.values(form).some((v) => !v)) return; const bookings = read('kopi-bookings', []); bookings.unshift({ ...form, id: Date.now(), status: 'Requested', created: new Date().toISOString() }); write('kopi-bookings', bookings); setDone(true) }
  return <main className="narrow-page"><div className="form-page"><p className="kicker">A seat for you</p><h1>Book a table.</h1><p className="muted">Send a request and we’ll confirm it manually. No real-time availability needed.</p>{done ? <div className="confirmation"><span>✓</span><h2>Request received.</h2><p>We’ll be in touch shortly to confirm your table.</p><Button href="#home">Back home</Button></div> : <form onSubmit={submit}><label>Name<input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required /></label><label>Party size<select value={form.party} onChange={(e) => setForm({ ...form, party: e.target.value })}>{[1,2,3,4,5,6,7,8].map((n) => <option key={n}>{n}</option>)}</select></label><div className="form-row"><label>Preferred date<input type="date" value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} required /></label><label>Time<input type="time" value={form.time} onChange={(e) => setForm({ ...form, time: e.target.value })} required /></label></div><label>Contact number<input type="tel" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} required /></label><button className="button" type="submit">Request booking <span>→</span></button></form>}</div></main>
}

function Admin({ user }) {
  if (user?.role !== 'staff') return <main className="narrow-page"><div className="empty-state"><h1>Staff only.</h1><p>Sign in with the staff demo account to view the counter dashboard.</p><Button href="#auth">Go to sign in</Button></div></main>
  const [orders, setOrders] = useState(() => read('kopi-orders', [])); const [bookings, setBookings] = useState(() => read('kopi-bookings', []))
  const update = (key, id, setter) => { const next = read(key, []).map((row) => row.id === id ? { ...row, status: 'Confirmed' } : row); write(key, next); setter(next) }
  return <main className="admin-page"><div className="admin-head"><div><p className="kicker">Counter view</p><h1>Good morning, team.</h1></div><span className="staff-badge">Staff mode</span></div><div className="admin-grid"><section><h2>Today’s orders <span>{orders.length}</span></h2>{orders.length ? orders.map((row) => <article className="admin-row" key={row.id}><div><strong>{row.user}</strong><p>{row.items.map((i) => `${i.qty}× ${i.item.name}`).join(', ')}</p></div><div className="row-action"><span className="status">{row.status}</span>{row.status !== 'Confirmed' && <button onClick={() => update('kopi-orders', row.id, setOrders)}>Fulfil</button>}</div></article>) : <p className="muted">No orders yet.</p>}</section><section><h2>Today’s bookings <span>{bookings.length}</span></h2>{bookings.length ? bookings.map((row) => <article className="admin-row" key={row.id}><div><strong>{row.name}</strong><p>Party of {row.party} · {row.date} at {row.time}</p></div><div className="row-action"><span className="status">{row.status}</span>{row.status !== 'Confirmed' && <button onClick={() => update('kopi-bookings', row.id, setBookings)}>Confirm</button>}</div></article>) : <p className="muted">No bookings yet.</p>}</section></div></main>
}
...
function Game({ user }) {
  ...
  const start = () => { setScore(0); setTime(60); setItems([]); setPopups([]); setParticles([]); setPlaying(true); last.current = 0; elapsed.current = 0; velocity.current = 0; itemsRef.current = []; moveCup(50) }
  ...
  useEffect(() => { if (!playing && time === 0) { const next = Math.max(best, score); setBest(next); write(`kopi-best-${user?.email}`, next) } }, [playing, time, score, best, user?.email])
...
function App() {
  const [currentRoute, setCurrentRoute] = useState(route()); const [user, setUser] = useState(() => read('kopi-user', null)); const [cart, setCartState] = useState(() => read('kopi-cart', []));
  ...
  const setCart = (next) => { setCartState(next); write('kopi-cart', next) }; const cartCount = useMemo(() => cart.reduce((sum, row) => sum + row.qty, 0), [cart]); const add = (item) => setCart([...cart.filter((r) => r.item.id !== item.id), { item, qty: (cart.find((r) => r.item.id === item.id)?.qty || 0) + 1 }])
  const signOut = () => { localStorage.removeItem('kopi-user'); setUser(null); window.location.hash = 'home' }
  let page = <Home onAdd={add} user={user} />
  if (currentRoute === 'auth') page = <Auth onAuth={setUser} />
  if (currentRoute === 'menu') page = <main className="narrow-page menu-page"><SectionHeading kicker="The menu">Local favourites, made with care.</SectionHeading><div className="menu-grid">{menuItems.map((item) => <MenuCard key={item.id} item={item} onAdd={add} />)}</div></main>
  if (currentRoute === 'order') page = <Order user={user} cart={cart} setCart={setCart} />
  ...
  return <><Header user={user} cartCount={cartCount} onSignOut={signOut} />{page}<footer><span>© 2025 Kopi &amp; Co.</span><span>Made for slow mornings.</span></footer></>
}
